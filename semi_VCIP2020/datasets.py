...
class VOCDataSet(data.Dataset):
    def __init__(self,
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(
                self.root, os.path.join(label_path, "{}.png".format(name)))
    ...
    def __getitem__(self, index):
        ...
        # === cv2.IMREAD_GRAYSCALE will load some additional wired value....
        ...
        name = datafiles["name"]
        # input("[datasets.py][88] np.unique(label):{}".format(np.unique(label)))
...
if __name__ == '__main__':
    dst = VOCDataSet("../psa/VOC2012",
                     list_path="../psa/voc12/train.txt",
                     label_path="RES_RW")
    trainloader = data.DataLoader(dst, batch_size=4)
    for i, data_ in enumerate(trainloader):
        imgs, labels, size, name = data_
        print("name: ", name, "len(imgs): ", len(imgs), "labels.shape: ",
              labels.shape)
        print(size)
